...
const Auth = ({type} : {type : "signup" | "signin"}) => {
  ...
  return (
    <div className="h-screen flex justify-center bg-stone-100 items-center flex-col">
        <div className="border border-stone-300 shadow-lg hover:animate-jump shadow-stone-400 w-4/5 lg:3/5 bg-stone-200 rounded-md flex flex-col">
            <div className="lg:text-3xl text-xl lg:text-2xl font-bold flex mx-2 flex-row text-slate-800 justify-center mt-3"> {type == "signin" ? "Account Login" : "Create account"}</div>
            <div className="text-slate-600 mx-2 text-sm lg:text-md flex flex-row justify-center">{type ==="signin" ?"Don't have an account?" : "Already have an account?"}<Link className="underline ml-2" to={type === "signin" ? "/signup" : "/signin"}>{type == "signin" ? "Sign Up" : "Sign In"}</Link></div>
...
function LabelledInput({title,placeholder,type,onChange} : LabelledInputType){
       return <div className="mx-4 lg:mx-8">
